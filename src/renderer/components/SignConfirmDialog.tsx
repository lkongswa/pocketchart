...
import { AlertTriangle, CheckCircle, X, XCircle, AlertOctagon } from 'lucide-react';
...
export default function SignConfirmDialog({
      ...
      <div key={issue.id} className={`rounded-lg border transition-all ${
        ...
        <div className="flex items-start gap-2 px-3 py-2.5">
          ...
          <div className="flex-1 min-w-0">
            ...
            {issue.fixable && !resolved && renderEditor(issue)}

            {!issue.fixable && issue.guidance && (
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
        <div className="bg-[var(--color-surface)] rounded-xl shadow-xl max-w-md w-full mx-4 overflow-hidden">
          <div className="flex items-center justify-between px-6 pt-5 pb-2">
            <h2 className="text-lg font-semibold text-[var(--color-text)]">Sign & Finalize This Document?</h2>
  ...
  if (isBlocked) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
        <div className="bg-[var(--color-surface)] rounded-xl shadow-xl max-w-lg w-full mx-4 overflow-hidden">
          <div className="flex items-center justify-between px-6 pt-5 pb-2">
            <h2 className="text-lg font-semibold text-red-700 flex items-center gap-2">
              <AlertOctagon className="w-5 h-5" /> Cannot Sign Document
            </h2>
            <button onClick={onClose} className="p-1 rounded hover:bg-gray-100 text-[var(--color-text-secondary)]">
              <X className="w-5 h-5" />
            </button>
      ...
      <div className="bg-[var(--color-surface)] rounded-xl shadow-xl max-w-2xl w-full mx-4 overflow-hidden max-h-[85vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-2 shrink-0">
          <h2 className="text-lg font-semibold text-[var(--color-text)]">
            {allErrorsResolved ? 'All Required Fields Complete' : 'Almost Ready \u2014 Fix These to Sign'}
          </h2>
          <button onClick={onClose} className="p-1 rounded hover:bg-gray-100 text-[var(--color-text-secondary)]">
            <X className="w-5 h-5" />
          </button>
