...
export default function PassphraseScreen({ onUnlock, showSetNewPassphrase }: PassphraseScreenProps) {
  ...
  const handleRecoveryUnlock = async () => {
    ...
    try {
      ...
      if (result.success) {
        // After recovery unlock, user must set a new passphrase
        setMode('set-new-passphrase');
      } else {
        setShaking(true);
        setTimeout(() => setShaking(false), 500);
        ...
        {mode === 'passphrase' && (
          <div className="bg-white rounded-2xl shadow-lg p-8">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h2 className="text-lg font-semibold text-[var(--color-text)]">Unlock Your Data</h2>
            </div>
            <p className="text-sm text-[var(--color-text-secondary)] mb-6">
              Enter your encryption passphrase to access your clinical records.
            </p>
